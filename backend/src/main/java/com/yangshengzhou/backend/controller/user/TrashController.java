...
public class TrashController {
     ...
     */
    @GetMapping("/folders")
    public ResponseEntity<ApiResponse<List<Folder>>> getTrashFolders(HttpServletRequest request) {
        try {
            User currentUser = authService.getCurrentUser();
            if (currentUser == null) {
                return ResponseEntity.status(401).body(ApiResponse.error("未登录"));
            ...
            List<Folder> folders = folderService.getTrashFolders(currentUser.getId());
            
            return ResponseEntity.ok(ApiResponse.success("获取回收站文件夹成功", folders));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error("获取回收站文件夹失败: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<String>> restoreFolder(@PathVariable Long id, HttpServletRequest request) {
        try {
            ...
            boolean restored = folderService.restoreFolder(id, currentUser.getId());
    ...
    public ResponseEntity<ApiResponse<String>> permanentDeleteFolder(@PathVariable Long id, HttpServletRequest request) {
        try {
            ...
            boolean deleted = folderService.permanentDeleteFolder(id, currentUser.getId());
